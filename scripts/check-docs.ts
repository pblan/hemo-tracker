...
for (const file of markdownFiles) {
  ...
  for (const match of source.matchAll(linkPattern)) {
    ...
    if (imageMarker === "!") {
      ...
      if (
        ...
        errors.push(
          `${displayFile}: image alternative text must describe the image`,
        );
      }
      ...
      if (!/^[a-z0-9]+(?:-[a-z0-9]+)*\.[a-z0-9]+$/.test(imageName)) {
        errors.push(
          `${displayFile}: image file name must use descriptive kebab-case: ${target}`,
        );
      }
    }
    ...
    if (!exists)
      errors.push(`${displayFile}: relative link does not exist: ${target}`);
  }
  ...
  for (const [index, match] of [...source.matchAll(mermaidPattern)].entries()) {
    try {
      ...
      errors.push(
        `${displayFile}: Mermaid block ${index + 1} is invalid: ${message}`,
      );
    }
  }
}
