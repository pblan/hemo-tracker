...
Google OAuth
     |
     v
Trusted desktop client  <---- HTTPS ---->  Self-hosted server
Tauri, React, Chakra UI                    Identity and opaque objects
     |                                           |
     v                                           v
Encrypted local vault                    SQLite metadata + ciphertext files
SQLCipher + encrypted source files
```
