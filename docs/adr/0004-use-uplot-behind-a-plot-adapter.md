...
# Use uPlot behind a plot adapter

## Context

Hemo Tracker must show 20 longitudinal laboratory plots at one time. Each plot must support irregular dates, changing source reference intervals, personal target ranges, source flags, and comparability boundaries. The first-render target is 500 ms. The pointer-frame latency target is 50 ms.

Issue 8 compared uPlot 1.6.32 and Apache ECharts 6.1.0 with the same browser, fixture, accessible table, and keyboard workflow. The fixture contained 1,000 lab reports, 100,000 measurements, 250 analytes, and 20 visible plots. The throwaway prototype is in branch [`prototype/plot-engine-benchmark`](https://github.com/pblan/hemo-tracker/tree/prototype/plot-engine-benchmark/apps/desktop/src/plot-engine-prototype).
...
Keep uPlot behind a local plot adapter. Feature code supplies normalized points, source reference intervals, personal target ranges, source flags, comparability boundaries, and theme tokens. Feature code must not supply uPlot option objects.
...
The desktop bundle and plot memory use stay small. The first-render and pointer-frame latency measurements meet the V1 targets in the product benchmark.
