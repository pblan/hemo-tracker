...
A decrypted export is a plaintext copy. Use it only when you need to share data with a trusted person or tool. Check the destination before export. Delete the export after use. Do not place it in a shared folder or cloud service unless that service is approved for the data.
